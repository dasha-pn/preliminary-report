...
Global planning uses NavFn (Dijkstra) over a \qty{0.05}{\metre} resolution 2D costmap. Local motion tracks the global path while reacting in real time to terrain changes and newly detected obstacles. Collision avoidance --- including dynamic hazards like other rovers is enforced through continuous obstacle detection and safety-margin inflation.
...
If the path becomes obstructed, the navigation state machine (\autoref{fig:navigation_combined}) performs local recovery, triggers global replanning if needed, or executes a fail-safe stop if no safe trajectory is found within a timeout.
\begin{figure}[H]
  \centering
  ...
  \begin{minipage}{0.49\linewidth}
    \centering
    \resizebox{\linewidth}{!}{\input{teams/navigation/figures/behavior_state_machine}}
  \end{minipage}

  \caption{Navigation overview: state machine and data flow}\label{fig:navigation_combined}
\end{figure}